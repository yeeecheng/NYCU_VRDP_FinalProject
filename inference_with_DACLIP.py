...
def main():
    ...
    checkpoint_path = "/swim-pool/yicheng/NYCU_VRDP_FinalProject/weights/wild-daclip_ViT-L-14.pt"
    daclip_encoder, preprocess = open_clip.create_model_from_pretrained("daclip_ViT-L-14", pretrained=checkpoint_path)
    daclip_encoder.eval().to(device)
...
def test(img_lq, model, args, window_size, idx, image_features=None, degra_features=None):
    if args.tile is None:
        # test the image as a whole
        # output, feature_maps = model(img_lq)
        output = model(img_lq, image_features, degra_features)
